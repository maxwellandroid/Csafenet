...
public class TechnicalSafetyActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_technical_safety);

        initializeViews();

        new GetTechnicalSafetyDetails().execute();
    }
    public  void back(View view){
        onBackPressed();
    }

    private class GetTechnicalSafetyDetails extends AsyncTask<String, Void,String> {

        ProgressDialog pDialog=new ProgressDialog(TechnicalSafetyActivity.this);
        String result="";
        RequestQueue requestQueue = Volley.newRequestQueue(TechnicalSafetyActivity.this);
        @Override
        protected String doInBackground(String... strings) {
            StringRequest MyStringRequest = new StringRequest(Request.Method.POST, StringConstants.mainUrl + StringConstants.technicalSafetyUrl, new Response.Listener<String>() {
                            ...
                            if(jsonObject.has("Policys")){
                                ...
                                if(policyDetailsModelList.size()>0){
                                    for (int i=0;i<policyDetailsModelList.size();i++){
                                        final  ImageView imageView=new ImageView(TechnicalSafetyActivity.this);
                                        Glide.with(TechnicalSafetyActivity.this)
                                    ...
                                    final TextView rowTextView = new TextView(TechnicalSafetyActivity.this);
