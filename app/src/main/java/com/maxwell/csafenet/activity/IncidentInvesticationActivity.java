...
public class IncidentInvesticationActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_incident_investication);

        initializeViews();

        new GetIncidentInvestigationDetails().execute();
    }
    public  void back(View view){
        onBackPressed();
    }
    private class GetIncidentInvestigationDetails extends AsyncTask<String, Void,String> {

        ProgressDialog pDialog=new ProgressDialog(IncidentInvesticationActivity.this);
        String result="";
        RequestQueue requestQueue = Volley.newRequestQueue(IncidentInvesticationActivity.this);
        @Override
        protected String doInBackground(String... strings) {
            StringRequest MyStringRequest = new StringRequest(Request.Method.POST, StringConstants.mainUrl + StringConstants.incidentReportingInvestigationUrl, new Response.Listener<String>() {
                            ...
                            if(jsonObject.has("Policys")){
                                ...
                                if(policyDetailsModelList.size()>0){
                                    for (int i=0;i<policyDetailsModelList.size();i++){
                                        final  ImageView imageView=new ImageView(IncidentInvesticationActivity.this);
                                        Glide.with(IncidentInvesticationActivity.this)
                                    ...
                                    final TextView rowTextView = new TextView(IncidentInvesticationActivity.this);
